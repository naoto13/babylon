...
生成経路（image-to-3D）の現行は **windows-image-to-3d** スキルを参照。本スキルは生成後の圧縮・組み込み工程を扱う。

> 注意: `tools/check-glb.mjs` は gltfpack 圧縮後（`EXT_meshopt_compression` 付き）の glb を読めない（`Invalid typed array length` で失敗する。2026-08-16 実測）。健全性チェックは**圧縮前**の glb に対して行い、圧縮後の検証は asset-preview ページで行うこと。
